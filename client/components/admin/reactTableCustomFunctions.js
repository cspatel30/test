...
export const _setTableStateWithPagination = (tableStatesCustom, list) => {
    tableStatesCustom.rows = list.content;
    tableStatesCustom.totalRecords = list.totalPages;
    tableStatesCustom.pageSize = list.size;
    tableStatesCustom.totalPagesWithRecords = Math.ceil(list.totalPages/list.size);
    tableStatesCustom.dataTableLoading = false;
    return tableStatesCustom;
}

export const _getSelectedRecordsKeyArray = (selected) => {
    let newSelectedRecords = [];
    for (var key in selected) {
      newSelectedRecords.push(parseInt(key));
    }
